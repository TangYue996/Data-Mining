...
def loadDataSet(filename):
    ...
    sub_element = element.getElementsByTagName("title")
    m = len(sub_element)
    print(m)
    print(sub_element[5].firstChild.data)
    data = []
    for i in range(m-1):
        print(sub_element[i].firstChild.data)
        data.append(sub_element[i].firstChild.data)
    return data
...
def wordSplit(data):
    ...
    stopwords = [line.strip() for line in open('F:/Data_Mining/Data_set/NYSK/stopwords.txt','r',encoding = 'utf-8').readlines()]
...
if __name__ == "__main__":
    ...
    data = loadDataSet('F:/Data_Mining/Data_set/NYSK/nysk.xml')
    #print(data)
    dataSplit = wordSplit(data)
    print('分词完成')
    saveFile('F:/Data_Mining/Data_set/NYSK/word-split.txt', dataSplit)  # 保存分词结果
